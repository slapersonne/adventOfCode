...
def run_15a():
    ...
    while not is_ended and i <= 5000:
        ...
        available_directions = {key: (direction, value) for key, (direction, value) in directions.items() if value != -1}
        best_direction = sorted(available_directions, key= lambda key: available_directions[key][1])[0]
        ...
        i += 1
        is_ended = len([(i, j) for i in range(-20, 19) for j in range(-18, 21) if (i, j) != (1, -1) and (i, j)not in map]) == 0
    return map
...
def run_15b():
    ...
    i = 0
    while True:
        not_filled = [position for position, value in map.items() if value not in ["O", -1]]
        if len(not_filled) == 0:
            break
        to_fill = [
            (x, y)
            for x, y in not_filled
            if map[(x+1, y)] == "O" or map[(x-1, y)] == "O" or map[(x, y+1)] == "O" or map[(x, y-1)] == "O"
        ]
