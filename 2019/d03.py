...
wires_coordinates = dict()


def extract_instruction(input):
    instruction_regex = re.compile("^([A-Z]{1})(\d+)$")
    matches = instruction_regex.match(input)
...
def run_03(input_wires):
    for index, wire in enumerate(input_wires):
        current_pos = (0, 0)
        steps = 0
        for instruction in wire:
            direction, number = extract_instruction(instruction)
            for i in range(1, number + 1):
                ...
                if current_pos in wires_coordinates:
                    if index == 1 and "A" in wires_coordinates[current_pos][0] and not "B" in wires_coordinates[current_pos][0]:
                        wires_coordinates[current_pos][1] += steps
                    wires_coordinates[current_pos][0] = f"{wires_coordinates[current_pos][0]}{letters[index]}"

                else:
                    wires_coordinates[current_pos] = [letters[index], steps]

    intersections = {position: value for position, value in wires_coordinates.items() if "A" in value[0] and "B" in value[0]}
    return intersections


def run_03a():
    intersections = run_03(wires)
    distances = [get_manhattan_distance(position) for position in intersections.keys()]
    distances.sort()
    print(distances)
...
def run_03b():
    intersections = run_03(wires)
    steps = [value[1] for value in intersections.values()]
    steps.sort()
    print(steps)
