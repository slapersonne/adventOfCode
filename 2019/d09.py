...
def run_09(instructions, global_inputs=[], i=0):
    global_outputs = []
    relative_pos = 0

    def get_parameter(parameter_mode, parameter_index, address_mode = False):
        if parameter_mode == 0:
            address = instructions.get(i + parameter_index, 0)
        elif parameter_mode == 1:
            address = i + parameter_index
        else:
            address = relative_pos + instructions.get(i + parameter_index, 0)
        return address if address_mode else instructions.get(address, 0)

    while True:
        instruction_input = instructions.get(i, 0)
        ...
        if operator == 99:
            ...
            input1 = get_parameter(first_parameter_mode, 1)
            input2 = get_parameter(second_parameter_mode, 2)
            output_adr = get_parameter(third_parameter_mode, 3, address_mode=True)
            output = input1 + input2 if operator == 1 else input1 * input2
            instructions[output_adr] = output
            i += 4

        elif operator == 3:
            parameter = get_parameter(first_parameter_mode, 1, address_mode=True)
            ...
            parameter = get_parameter(first_parameter_mode, 1)
            if parameter != 0:
                i = get_parameter(second_parameter_mode, 2)
            else:
                i += 3
...
def run_09a():
    instructions = {index: instruction for index, instruction in enumerate(initial_instructions)}
    _, _, output = run_09(instructions, [2])
    print(output)
