...
def extract_note(note):
    note_regex = re.compile("^\[\d{4}-\d{2}-(\d{2}) (\d{2}):(\d{2})\] (Guard #(\d+) begins shift)?(falls asleep)?(wakes up)?$")
    matches = note_regex.match(note)
    return int(matches.group(1)), \
           int(matches.group(2)), \
           int(matches.group(3)), \
           matches.group(4) is not None, \
           int(matches.group(5)) if matches.group(5) is not None else None, \
           matches.group(6) is not None, \
           matches.group(7) is not None

def print_guard_watch(guard_id, day):
    print(f'Guard {guard_id} : {"".join(["." if day[i] else "Z" for i in day.keys()])}')
...
def _run_04():
    ...
    guard_watches = dict()
    for note in notes:
        day, hour, minute, is_shift, guard_id, is_sleep, is_wake = extract_note(note)
        if guard_id is not None:
            current_guard = guard_id
        if is_shift:
            new_day = {i: True for i in range(0, 60)}
            if current_guard in guard_watches.keys():
                guard_watches[current_guard].append(new_day)
            else:
                guard_watches[current_guard] = [new_day]
        if is_sleep:
            current_day = guard_watches[current_guard].pop()
            if hour != 0:
                minute = 0
            fixed_current_day = {i: current_day[i] if i < minute else False for i in range(0, 60)}
            guard_watches[current_guard].append(fixed_current_day)
        if is_wake:
            current_day = guard_watches[current_guard].pop()
            if hour != 0:
                minute = 0
            fixed_current_day = {i: current_day[i] if i < minute else True for i in range(0, 60)}
            guard_watches[current_guard].append(fixed_current_day)
    return guard_watches


def run_04a():
    guard_watches = _run_04()

    guards_asleep_minutes = dict()
    guards_minutes_slept = dict()

    for guard, watches in guard_watches.items():
        minutes_asleep = 0
        minutes_slept = {i: 0 for i in range(0, 60)}
        for watch in watches:
            minutes_asleep += len([value for value in watch.values() if not value])
            for i in watch.keys():
                if not watch[i]:
                    minutes_slept[i] += 1
        # print(f"Guard #{guard} : {minutes_asleep} ({minutes_slept})")
        guards_asleep_minutes[guard] = minutes_asleep
        guards_minutes_slept[guard] = minutes_slept

    maximum_asleep_minutes = -1
    maximum_guard = -1
    maximum_minute = -1
    for guard in guards_asleep_minutes.keys():
        guard_asleep_minutes = guards_asleep_minutes[guard]
        if guard_asleep_minutes > maximum_asleep_minutes:
            maximum_asleep_minutes = guard_asleep_minutes
            print(f"New maximum : Guard #{guard} with {guard_asleep_minutes} minutes ({guards_minutes_slept[guard]})")
            maximum_guard = guard
            maximum_minutes_slept = -1
            for key, value in guards_minutes_slept[guard].items():
                if value > maximum_minutes_slept:
                    maximum_minute = key
                    maximum_minutes_slept = value
                    print(f"New maximum : Minute {key} ({value})")
    print(f"Total : {maximum_guard} * {maximum_minute} = {maximum_guard * maximum_minute}")
...
def run_04b():
    ...
    maximum_minute = -1
    maximum_slept = -1
    for guard, minutes_slept in guards_minutes_slept.items():
        for minute, slept in minutes_slept.items():
            if slept > maximum_slept:
                print(f"New maximum : Guard #{guard} slept minute {minute} {slept} times !")
                maximum_slept = slept
                maximum_guard = guard
