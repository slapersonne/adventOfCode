...
def parse_mask(line, ignore_x=False):
    mask = line[7:]
    return {
        36 - i: c
...
def get_updated_value(value, mask):
...
def get_updated_addresses(address, mask):
    ...
    for b in masked_address:
        new_addresses = []
        for start in (addresses if len(addresses) else [""]):
...
def run_14a():
    ...
    for line in lines:
        if line.startswith("mask"):
            ...
            value = get_updated_value(value, mask)
...
def run_14b():
    ...
    for line in lines:
        if line.startswith("mask"):
            ...
            address, value = parse_mem(line)
            addresses = get_updated_addresses(address, mask)
